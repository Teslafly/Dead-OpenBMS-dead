...
typedef struct BurstDataType{ // this needs to be flippped!
 ...
 uint8_t SPIbuffer[14]; // spi databuffer. 1 byte for control, 14 bytes for recieved / transmitted data.
...
uint16_t ATA68_readCell(uint8_t cell, uint8_t board){ // reads individual cell - this works!
 ...
 if((cell >=0) && (cell < 8)){ // error checking. The first 5 msb's must remain 0.
  ...
  ATA68_WRITE(board, OpReq, (OpReq_Val | 1)); // set operation request bit to start conversion.
  ...
  uint8_t *SPIbuffer = ATA68_READ(board, statusReg, 1);
  if (SPIbuffer[1] | 0x01) // check if dataready bit is set.
  {
...
{
  ATA68_WRITE(board, OpReq, (OpReq_Val | B00001011 )); // set operation request bit to start conversion.
  ...
  } // find somthing that will retry the communication.
...
/////////////////////////////////////
void ATA68_ResistorControl(uint8_t device, uint8_t cells){
  ...
  /// write oprqst register
}

//////////////////////////////////////////////////
byte ATA68_GetOpStatus(uint8_t board)// get operation status
{ //useage
  // output:
  // 0 = no operation
  // 1 = ongoing operation
  ...
  uint8_t *SPIbuffer = ATA68_READ(board, Opstatus, 1);
  
  uint8_t opstatus = SPIbuffer[1];
  return opstatus;
}



byte getStatus(uint8_t board)
{
  uint8_t *SPIbuffer = ATA68_READ(board, statusReg, 1);
...
void ATA68_SetUdvTrip(int LowVoltege) // some trickery has to go on with this one. not currently used.
{
...
{ 
  ...
  ATA68_WRITE(board, ChannelDischSel, (OpReq_Val | 1)); // set operation request bit to start conversion.
}
